...
- (void)show_menu
{
  // Attach the menu to the status item and trigger it via the button, so AppKit
  // positions it natively directly below the menu bar. The original
  // popUpMenuPositioningItem:atLocation:inView: call placed the menu at the
  // button's zero origin, which rendered it over the menu-bar icons until a
  // mouse move forced a relayout. The menu is detached again afterwards so the
  // next click follows the same path.
